...
async def ws_handler(request: web.Request):
    ...
    async def send_info(msg: str):
        if ws.closed: return # 28호에서 try-except로 감싸져 있었으나, 29호에서 간략화됨. ws.closed 체크로 충분.
    ...
    async def process_transcription_buffer(is_final=False):
        ...
        async with processing_lock:
            ...
            # 28호에서 initial_prompt는 client_whisper_opts에 포함되어 있었고,
            # transcribe 함수 호출 시 **client_whisper_opts를 사용했으므로 자동으로 적용되었음.
            # 29호에서 pop하고 다시 넣어주는 방식은 기능적으로 큰 차이는 없으나,
            # 대원칙 1, 2에 따라 기존 방식 유지.
            # transcribe_opts.pop('initial_prompt', None) # 29호의 이 라인 삭제
        ...
        while not ws.closed:
            ...
            if msg.type == web.WSMsgType.BINARY:
                ...
                if data.get("type") == "get_config": await send_config()
                    ...
                    # 28호에서 추가된 translation 설정 변경 로그
                    if "translation" in opts:
                        engine_updated = client_config.get("translation", {}).get("engine")
                        langs_updated = client_config.get("translation", {}).get("target_langs")
                        logging.info(f"[{log_id}] Translation config updated: engine={engine_updated}, target_langs={langs_updated}")
