...
DEFAULT_CONFIG = {
    "whisper": {
        ...
        "log_prob_threshold": -0.3, "compression_ratio_threshold": 1.6, "beam_size": 5, "best_of": 1,
    ...
    "vad": {
        ...
        "max_silence_ms": 700, "trail_pad_ms": 160, "max_segment_sec": 10,
        "buffer_flush_length": 30, "flush_timeout": 1.2
    },
...
async def ws_handler(request: web.Request):
    ...
    sentence_buffer = ""
    buffer_flush_length = client_config.get("vad", {}).get("buffer_flush_length", 30) # config 값 참조
    flush_timeout = client_config.get("vad", {}).get("flush_timeout", 1.2) # config 값 참조
    ...
    try:
        while not ws.closed:
            ...
            # [수정] '지능형 타임아웃' 로직: 타임아웃 발생 시에만 짧은 문장을 처리
