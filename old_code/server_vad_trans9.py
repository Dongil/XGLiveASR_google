...
# [수정] 최적화된 파라미터를 기본 설정(DEFAULT_CONFIG)으로 반영합니다.
DEFAULT_CONFIG = {
    "whisper": {
        "language": "ko", "task": "transcribe", "temperature": [0.0, 0.2],
        "condition_on_previous_text": True,  # 문맥 유지를 위해 True로 변경
        "no_repeat_ngram_size": 3, "repetition_penalty": 1.2,
        "log_prob_threshold": -0.4, # 환각 억제를 위해 수정
        "compression_ratio_threshold": 1.6, "beam_size": 3, "best_of": 1,
        "no_speech_threshold": 0.6, # 환각 억제를 위해 추가
        "initial_prompt": "재난, 지진, 개요, 다수사상자, 중증도 분류, 트리아즈, Triage, 스타트 분류, START"
    },
    "vad": {
        "engine": "silero", "threshold_on": 0.28, "threshold_off": 0.25,
        "window_ms": 400, "pre_speech_ms": 280, "min_speech_ms": 120,
        "max_silence_ms": 500,  # 반응성 개선을 위해 수정
        "trail_pad_ms": 160, "max_segment_sec": 7, # 반응성 개선을 위해 수정
        "buffer_flush_length": 25, "flush_timeout": 1.0 # 반응성 개선을 위해 수정
    },
...
async def ws_handler(request: web.Request):
    ...
    sentence_buffer = ""
    buffer_flush_length = client_config.get("vad", {}).get("buffer_flush_length", 40)
    flush_timeout = client_config.get("vad", {}).get("flush_timeout", 1.2)
    ...
    # [추가] 이중 버퍼링 로직을 위한 변수 선언
    transcribe_buffer = bytearray()
    MIN_CHUNK_SECONDS = 3.0  # Whisper에 최소 3초 이상의 오디오를 전달하여 정확도 확보
    ...
    try:
        while not ws.closed:
            try:
                        ...
                        if chunk:
                            ...
                            if len(transcribe_buffer) >= MIN_CHUNK_BYTES:
                                ...
                                if text_fragment:
                                    ...
                                    elif len(sentence_buffer) > buffer_flush_length:
                                        await send_stt(sentence_buffer.strip())
                                    else:
                                        sentence_buffer = sentences[0] if sentences else ""
                ...
                # [수정] 타임아웃 발생 시, 인식 버퍼에 남아있는 데이터도 처리
                ...
                if sentence_buffer.strip():
                    print(f"[server] Timeout reached. Flushing buffer: {sentence_buffer.strip()}")
                    await send_stt(sentence_buffer, is_final_flush=True)
                continue
    finally:
        print("[server] Connection closing, flushing remaining buffers...")
        # [수정] 종료 시, collector와 인식 버퍼에 남은 모든 데이터를 합쳐서 최종 처리
