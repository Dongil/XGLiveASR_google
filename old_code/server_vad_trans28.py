# 버전 28 - '문장 인식(KSS) 토글 기능'을 추가한 버전
import asyncio
import json
import os
import ssl
...
async def ws_handler(request: web.Request):
    ...
    try:
        ...
        if len(transcribe_buffer) > BYTES_PER_FRAME:
            ...
            text_fragment = "".join(s.text for s in segments).strip()
            text_fragment = re.sub(r'[^\x20-\x7E가-힣\s.,?!]', '', text_fragment)
            if text_fragment: await send_stt(text_fragment, is_final_flush=True) # [수정] 마지막 flush 시 is_final_flush=True 전달
