...
async def cors_mw(request, handler):
    ...
    allowed_origins = { "http://localhost:8080", "http://1227.0.0.1:8080", "https://asr.xenoglobal.co.kr", "https://asr.xenoglobal.co.kr:8448" }
...
async def ws_handler(request: web.Request):
    ...
    buffer_flush_length = vad_cfg.get("buffer_flush_length", 30)
    ...
    MIN_CHUNK_SECONDS = 1.5 # [수정됨] 반응성을 높이기 위해 3.5초에서 1.5초로 조정
    ...
    try:
        while not ws.closed:
            ...
            # [수정됨] TimeoutError 처리 로직 개선
                ...
                if text_fragment or sentence_buffer:
                    print(f"[server] Timeout reached. Flushing all buffers...")
                    await send_stt(text_fragment, is_final_flush=True)
                
                continue
    # [추가됨] 비정상 종료를 포함한 모든 예외 처리
    except Exception as e:
        logging.warning(f"[server] WebSocket handler caught an exception: {e}")
    finally:
        # [수정됨] 마지막 정리 로직은 그대로 유지, 어떤 상황에서도 실행되도록 보장
