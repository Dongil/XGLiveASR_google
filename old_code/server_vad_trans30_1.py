# 버전 30 - '문장 인식(KSS) 토글 기능'을 추가한 버전
# [수정] 동일 ID 접속자에게 메시지를 방송(Broadcast)하는 기능 추가
import asyncio
import json
import os
import ssl
...
import uuid
import copy
from typing import Optional, Deque, Dict, List, Set
...
# [추가] 모든 클라이언트 연결을 관리하기 위한 전역 변수
# 구조: {'id_A': {ws_obj1, ws_obj2}, 'id_B': {ws_obj3}}
...
async def ws_handler(request: web.Request):
    ...
    if user_id_raw:
        ...
        # [수정] ID가 없는 경우를 명확히 처리하고, 방송을 위해 고유 ID 부여
        user_id = f"anonymous_{str(uuid.uuid4().hex)[:8]}"

    # [추가] 클라이언트를 전역 CLIENTS 딕셔너리에 등록
    if user_id not in CLIENTS:
        CLIENTS[user_id] = set()
    CLIENTS[user_id].add(ws)
    
    log_id = f"{user_id} ({len(CLIENTS[user_id])})" # 로그에 현재 접속자 수 표시
    ...
    # [수정] _send_single_sentence를 broadcast_sentence로 변경하여 같은 ID의 모든 클라이언트에게 전송
    async def broadcast_sentence(text: str):
        text = text.strip()
        if not text: return
        ...
        response_str = json.dumps({"type": "stt", "text": text, "translations": translations}, ensure_ascii=False)
        
        # 현재 user_id에 해당하는 모든 활성 ws 연결에 메시지 전송
        if user_id in CLIENTS:
            # 연결이 끊어진 클라이언트를 제거하기 위해 복사본을 순회
            current_clients = list(CLIENTS[user_id])
            tasks = [client.send_str(response_str) for client in current_clients if not client.closed]
            if tasks:
                await asyncio.gather(*tasks, return_exceptions=True) # 예외 발생 시에도 다른 전송은 계속
        
        print(f"[{log_id}] SENT to {len(tasks)} clients: {text} | Translated: {translations}")

    # [수정] send_stt가 broadcast_sentence를 호출하도록 수정
    async def send_stt(text_to_send: str, is_final_flush=False):
        ...
        if not use_kss:
            ...
            if text_fragment:
                await broadcast_sentence(text_fragment)
            return
        ...
        if is_final_flush and sentences:
            for sentence in sentences:
                await broadcast_sentence(sentence)
            sentence_buffer = ""
            return
    ...
    last_speech_time = loop.time() # <<<--- 이 줄을 추가하세요    

    async def silence_watcher():
        nonlocal last_audio_time, last_speech_time # <<<--- last_speech_time 추가
        while not ws.closed:
            await asyncio.sleep(0.5)
            now = loop.time()
            # vvv--- 아래 if 조건문을 통째로 교체하세요 ---vvv
            if (now - last_speech_time > flush_timeout) and (collector.collected or transcribe_buffer or sentence_buffer):
    ...
    try:
        ...
        # [수정] 연결 종료 시 반드시 CLIENTS 목록에서 자신을 제거
        if user_id in CLIENTS:
            CLIENTS[user_id].discard(ws)
            if not CLIENTS[user_id]: # 해당 ID에 더 이상 클라이언트가 없으면 ID 자체를 삭제
                del CLIENTS[user_id]
        ...
        if len(transcribe_buffer) > BYTES_PER_FRAME:
            ...
            text_fragment = "".join(s.text for s in segments).strip()
            text_fragment = re.sub(r'[^\x20-\x7E가-힣\s.,?!]', '', text_fragment)
            if text_fragment: await send_stt(text_fragment, is_final_flush=True)
        if sentence_buffer:
            await send_stt("", is_final_flush=True)
        
        print(f"[{log_id}] ws client disconnected. Remaining clients for '{user_id}': {len(CLIENTS.get(user_id, []))}")
