# 25호 소스 - 비동기 잠금(Lock)을 도입하여 중복 출력 버그를 해결한 최종 안정화 버전
import asyncio
import json
import os
import ssl
...
async def ws_handler(request: web.Request):
    ...
    # [추가됨] 동시 처리를 방지하기 위한 비동기 잠금(Lock)
    ...
    async def silence_watcher():
        ...
        while not ws.closed:
            ...
            if (now - last_audio_time > flush_timeout) and (collector.collected or transcribe_buffer or sentence_buffer):
                # [수정됨] 잠금(Lock)을 획득하여 메인 루프의 처리와 겹치지 않도록 함
                async with processing_lock:
                    # Lock 획득 후 다시 한 번 조건을 체크하여 이중 처리를 방지
                    if (now - last_audio_time > flush_timeout) and (collector.collected or transcribe_buffer or sentence_buffer):
                        print(f"[server] Silence detected. Flushing all buffers...")
            ...
            if msg.type == web.WSMsgType.BINARY:
                ...
                while len(buf) >= BYTES_PER_FRAME:
                    ...
                    if len(transcribe_buffer) >= MIN_CHUNK_BYTES:
                        # [수정됨] 잠금(Lock)을 획득하여 '침묵 감시병'과의 충돌을 방지
                        async with processing_lock:
                            # Lock을 획득했으므로, 현재 버퍼 내용을 안전하게 처리
                            arr16 = np.frombuffer(bytes(transcribe_buffer), dtype=np.int16)
                            transcribe_buffer.clear()
                            f32 = int16_to_float32(arr16)
                            segments, _ = model.transcribe(f32, **client_whisper_opts)
