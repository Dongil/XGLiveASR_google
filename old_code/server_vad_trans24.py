# 24호 소스 - 재접속 시 VAD 상태를 초기화하여 연결 안정성을 확보한 최종 완성본
import asyncio
import json
import os
import ssl
...
async def ws_handler(request: web.Request):
    ...
    # [추가됨] 새로운 연결이 시작될 때마다 VAD의 내부 상태를 깨끗하게 초기화
    silero.reset()
        ...
        while not ws.closed:
            ...
            if msg.type == web.WSMsgType.BINARY:
                ...
                if data.get("type") == "get_config": await send_config()
                    ...
                    if "vad" in opts:
                        vad_cfg = client_config["vad"]
                        # VAD 설정 변경 시 silero 모델 자체를 새로 만드는 것은 비효율적이므로,
                        # 파라미터만 업데이트하는 것이 좋으나 현재 구조에서는 클래스를 새로 생성함.
                        # 재접속 문제가 더 중요하므로 일단 유지.
