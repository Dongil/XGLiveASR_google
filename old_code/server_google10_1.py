...
import uuid
import copy
from typing import Optional, Dict, Set # [수정] Set 추가
...
# (이전 버전과 동일한 나머지 모든 코드 - 번역기 클래스, 설정 로드, 웹소켓 핸들러 등)
# ...
...
# [추가] 모든 클라이언트 연결을 관리하기 위한 전역 변수 (server_vad_trans30.py 참고)
...
async def ws_handler(request: web.Request):
    ...
    # [추가] 클라이언트를 전역 CLIENTS 딕셔너리에 등록 (server_vad_trans30.py 참고)
    ...
    # [수정] send_sentence_with_translation을 broadcast 기능으로 변경 (server_vad_trans30.py 참고)
                    ...
                    if final_piece:
                        ...
                        if len(sentences) > 1:
                            for sentence in sentences[:-1]:
                                await broadcast_sentence_with_translation(sentence) # [수정] broadcast 함수 호출
    ...
    try:
        ...
        if sentence_buffer.strip():
            await broadcast_sentence_with_translation(sentence_buffer) # [수정] broadcast 함수 호출
        
        google_task.cancel()
        try: await google_task
        except asyncio.CancelledError: pass
        
        # [추가] 연결 종료 시 반드시 CLIENTS 목록에서 자신을 제거 (server_vad_trans30.py 참고)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info("[server] Server is now fully ready and listening on ws://0.0.0.0:8000")
