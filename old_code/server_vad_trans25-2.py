# 버전 25.2 - 25호 원본 기준, 안정적 ID 로깅 기능만 추가한 최종 버전
import asyncio
import json
import os
import ssl
import logging
import re
import uuid  # [추가] 클라이언트별 고유 ID 생성을 위해 추가
...
async def ws_handler(request: web.Request):
    ...
    print(f"[{client_id}] ws client connected from {request.remote}")
    ...
    async def _send_single_sentence(text: str):
        ...
        try:
            ...
            # [수정] 로그에 client_id 추가
            print(f"[{client_id}] SENT: {text} | Translated: {translations}")
        except Exception as e:
            logging.warning(f"[{client_id}] Could not send message, connection likely closed: {e}")
        ...
        while not ws.closed:
            ...
            if (now - last_audio_time > flush_timeout) and (collector.collected or transcribe_buffer or sentence_buffer):
                async with processing_lock:
                    if (now - last_audio_time > flush_timeout) and (collector.collected or transcribe_buffer or sentence_buffer):
                        # [수정] 로그에 client_id 추가
                        print(f"[{client_id}] Silence detected. Flushing all buffers...")
    ...
    try:
            ...
            if msg.type == web.WSMsgType.BINARY:
                ...
                if data.get("type") == "get_config": await send_config()
                    ...
                    if "vad" in opts:
                        ...
                        flush_timeout = vad_cfg.get("flush_timeout", 1.4)
                        logging.info(f"[{client_id}] VAD config updated.")
        ...
        logging.warning(f"[{client_id}] WebSocket handler caught an exception: {e}")
    finally:
        watcher_task.cancel()
        try:
            await watcher_task
        ...
        # [수정] 로그에 client_id 추가
        print(f"[{client_id}] ws client disconnected")
