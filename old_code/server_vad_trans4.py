...
async def ws_handler(request: web.Request):
    ...
    async def buffer_timeout_task():
        nonlocal last_activity_time
        while not ws.closed:
            await asyncio.sleep(FLUSH_TIMEOUT / 2)
            if sentence_buffer.strip():
                if (asyncio.get_event_loop().time() - last_activity_time) > FLUSH_TIMEOUT:
    ...
    timeout_task = asyncio.create_task(buffer_timeout_task())
            ...
            if msg.type == web.WSMsgType.BINARY:
                ...
                if data.get("type") == "get_config":
                    ...
                    if "vad" in opts:
                        ...
                        buffer_flush_length = client_config.get("vad", {}).get("buffer_flush_length", 40)
                        logging.info(f"VAD config updated. Buffer flush length set to: {buffer_flush_length}")
