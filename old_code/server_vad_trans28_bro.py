# 버전 28.1 - 'Broadcasting 기능'을 추가한 버전
import asyncio
import json
import os
import ssl
...
# [신규] Broadcasting을 위한 클라이언트 관리 딕셔너리
...
async def ws_handler(request: web.Request):
    ...
    # [수정] Broadcasting을 위한 클라이언트 관리 로직 추가
    if user_id not in CLIENTS:
        CLIENTS[user_id] = set()
    CLIENTS[user_id].add(ws)
    
    # [수정] 로그 ID에 현재 접속자 수를 포함하여 더 명확하게 표시
    log_id = f"{user_id} ({len(CLIENTS[user_id])})" if user_id else f"anonymous ({str(uuid.uuid4().hex)[:8]})"
    ...
    # 아래 함수들은 특정 클라이언트(교수)에게만 응답을 보내므로 수정하지 않음
    ...
    # [수정] _send_single_sentence를 _broadcast_single_sentence로 변경하여 다중 클라이언트에 전송
    async def _broadcast_single_sentence(text: str):
        text = text.strip()
        if not text: return

        current_clients = CLIENTS.get(user_id, set())
        if not current_clients: return
        ...
        send_tasks = [client.send_str(json_payload) for client in current_clients if not client.closed]
        
        if send_tasks:
            await asyncio.gather(*send_tasks, return_exceptions=True)
            print(f"[{log_id}] BROADCAST: {text} | Translated: {translations} to {len(send_tasks)} clients.")
        else:
            logging.warning(f"[{log_id}] No active clients to broadcast to for user '{user_id}'.")


    # [수정] 내부적으로 _broadcast_single_sentence를 호출하도록 변경
    async def send_stt(text_to_send: str, is_final_flush=False):
        ...
        if not use_kss:
            ...
            if text_fragment:
                await _broadcast_single_sentence(text_fragment)
            return
        ...
        if is_final_flush and sentences:
            for sentence in sentences:
                await _broadcast_single_sentence(sentence)
            sentence_buffer = ""
            return
    ...
    try:
        ...
        # [수정] Broadcasting을 위한 클라이언트 관리 로직 추가 (연결 종료)
        ...
        print(f"[{log_id}] ws client disconnected. Remaining clients for '{user_id}': {remaining_clients}")
