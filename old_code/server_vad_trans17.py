...
# [수정] UtterCollector 클래스는 더 이상 사용되지 않습니다.
...
async def ws_handler(request: web.Request):
    ...
    # [핵심 수정] 무손실 데이터 처리를 위한 새로운 변수 선언
    is_speaking = False
    ...
    silence_counter = 0
    raw_audio_buffer = bytearray() # 클라이언트로부터 받은 raw 데이터를 임시 저장
    ...
    try:
        while not ws.closed:
            try:
                ...
                if is_speaking: # 말이 끝나지 않았는데 타임아웃이 걸린 경우, 강제로 말을 끝냄
                    is_speaking = False
