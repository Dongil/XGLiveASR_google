...
if not TRANSLATORS:
    logging.warning("사용 가능한 번역 엔진이 없습니다. API 키 설정을 확인하세요.") # 경고 수준으로 변경
else:
...
DEFAULT_CONFIG = {
    ...
    "vad": {
        ...
        "max_silence_ms": 600, "trail_pad_ms": 160, "max_segment_sec": 9
    },
    "model": {
    ...
    "translation": {
        ...
        # <--- [변경] target_lang를 target_langs 배열로 변경하여 다중 언어를 지원합니다.
                ...
                while len(buf) >= BYTES_PER_FRAME:
                    ...
                    if chunk:
                        ...
                        if text:
                            ...
                            # target_lang 대신 target_langs 배열을 가져옵니다.
                            ...
                            if engine_name and target_langs and engine_name in TRANSLATORS:
                                ...
                                # 각 언어에 대한 번역 작업을 비동기 태스크로 만듭니다.
                                translation_tasks = [
                                    translator.translate(text, lang) for lang in target_langs
                                ]
                                
                                # asyncio.gather를 사용하여 모든 번역을 병렬로 실행합니다.
                                if translation_tasks:
                                    translations = await asyncio.gather(*translation_tasks)
                            
                            # 응답 데이터 형식을 'translations' 배열로 변경합니다.
                            response_data = {"type": "stt", "text": text, "translations": translations}
                            await ws.send_str(json.dumps(response_data, ensure_ascii=False))
                            print(f"[server] STT: {text} | Translated: {translations}")
                        # <--- [변경 끝] ---
...
if __name__ == "__main__":
    ...
    # SSL 인증서 경로를 환경에 맞게 수정하세요.
    # 예: ssl_ctx.load_cert_chain("path/to/cert.pem", "path/to/key.pem")
    # 로컬 테스트 시에는 ssl_context=None 으로 설정하여 http로 실행할 수 있습니다.
    try:
        ...
        print("실제 서비스 환경에서는 반드시 SSL 인증서를 사용해야 합니다.\n")
