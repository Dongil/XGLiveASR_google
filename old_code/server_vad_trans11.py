...
DEFAULT_CONFIG = {
    "whisper": {
        ...
        "log_prob_threshold": -0.35, "compression_ratio_threshold": 1.6, "beam_size": 5, "best_of": 1,
    ...
    "vad": {
        ...
        "max_silence_ms": 600, "trail_pad_ms": 160, "max_segment_sec": 8,
        "buffer_flush_length": 25, "flush_timeout": 1
    },
...
async def ws_handler(request: web.Request):
    ...
    # [추가] '최소 문맥 보장'을 위한 상수 추가
    MIN_PROCESSING_SECONDS = 1.5
    MIN_PROCESSING_BYTES = int(MIN_PROCESSING_SECONDS * SAMPLE_RATE * 2)
            ...
            try:
                ...
                if msg.type == web.WSMsgType.BINARY:
                    ...
                    while len(buf) >= BYTES_PER_FRAME:
                        ...
                        # [수정] is_speech 상태가 변경될 때를 감지하기 위해 이전 상태 저장
                        prev_state_speech = state_speech
                        ...
                        # [수정] '최소 문맥 보장 플러시' 로직 적용
                        should_flush = False
                        # 1. 버퍼가 꽉 찼을 때
                        if len(transcribe_buffer) >= MIN_CHUNK_BYTES:
                            should_flush = True
                        # 2. 말이 끝났고(상태변경), 버퍼에 처리할 최소 분량이 있을 때
                        elif prev_state_speech and not state_speech and len(transcribe_buffer) >= MIN_PROCESSING_BYTES:
                            should_flush = True
