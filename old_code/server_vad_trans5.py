...
# ---------- 전역 설정/기본 (flush_timeout 추가) ----------
CONFIG_PATH = "config.json"
DEFAULT_CONFIG = {
    "whisper": { "language": "ko", "task": "transcribe", "temperature": [0.0, 0.2], "condition_on_previous_text": False, "no_repeat_ngram_size": 3, "repetition_penalty": 1.2, "log_prob_threshold": -0.2, "compression_ratio_threshold": 1.6, "beam_size": 6, "best_of": 1 },
    "vad": { 
        "engine": "silero", "threshold_on": 0.25, "threshold_off": 0.25, 
        "window_ms": 400, "pre_speech_ms": 280, "min_speech_ms": 120, 
        "max_silence_ms": 1000, "trail_pad_ms": 160, "max_segment_sec": 10, 
        "buffer_flush_length": 25, "flush_timeout": 1.0 
    },
                ...
                if msg.type == web.WSMsgType.BINARY:
                    ...
                    if data.get("type") == "get_config": await send_config()
                        ...
                        if "vad" in opts:
                            ...
                            buffer_flush_length = client_config.get("vad", {}).get("buffer_flush_length", 40)
                            flush_timeout = client_config.get("vad", {}).get("flush_timeout", 1.2) # [수정] flush_timeout도 업데이트
