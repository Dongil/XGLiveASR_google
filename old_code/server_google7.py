# 버전 32.5 - Google STT speech_adaptation 필드명 오류 수정
import asyncio
import json
import os
import ssl
...
async def ws_handler(request: web.Request):
    ...
    async def send_sentence_with_translation(sentence: str):
        sentence = sentence.strip()
        if not sentence: return
        sentence_id = str(uuid.uuid4())
        await send_json({"type": "stt_final", "sentence_id": sentence_id, "text": sentence})
        print(f"[{log_id}] SENT (Final Sentence): {sentence}")
        ...
        print(f"[{log_id}] SENT (Translation): {sentence} | Translated: {translations}")
    ...
    async def google_stream_processor():
        ...
        try:
            ...
            # [핵심 수정] adaptation_object를 올바른 필드 이름인 'adaptation'으로 전달
            ...
            print(f"[{log_id}] Starting new Google STT stream (single_utterance=True)...")
            ...
            async for response in stream:
                ...
                if result.is_final:
                    ...
                    if final_piece:
                        ...
                        print(f"[{log_id}] RECV (Final Utterance): {final_piece}")
                        sentence_buffer += final_piece + " "
                        sentences = kss.split_sentences(sentence_buffer)
                        if len(sentences) > 1:
                            for sentence in sentences[:-1]: await send_sentence_with_translation(sentence)
    ...
    try:
        ...
        if sentence_buffer.strip():
            await send_sentence_with_translation(sentence_buffer)
        google_task.cancel()
