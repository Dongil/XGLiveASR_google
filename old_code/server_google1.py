# 버전 31.1 - Google STT 실시간성 확보 (interim_results=True 적용)
import asyncio
import json
import os
import ssl
...
async def ws_handler(request: web.Request):
    ...
    async def process_stt_result(text: str, is_final: bool):
        ...
        # [수정] 최종 결과일 때만 번역을 수행하여 비용 절감 및 성능 향상
        translations = []
        if is_final:
        ...
        # [수정] 메시지 타입에 is_final 플래그 추가
        response = {"type": "stt", "text": text, "translations": translations, "is_final": is_final}
        try:
            await ws.send_str(json.dumps(response, ensure_ascii=False))
            # 로그는 최종 결과일 때만 출력하여 가독성 확보
            if is_final:
    ...
    async def google_stream_processor():
        ...
        try:
            ...
            # [수정] interim_results=True로 변경하여 실시간성 확보
            ...
            if adaptation_client and phrase_set_name:
                try:
                    ...
                    print(f"[{log_id}] Google STT Adaptation Phrase Set '{phrase_set_name}' deleted.")
                except Exception as e:
