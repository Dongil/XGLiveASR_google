...
DEFAULT_CONFIG = {
    ...
    "vad": {
        "engine": "silero", "threshold_on": 0.3, "threshold_off": 0.25,
        "window_ms": 400, "pre_speech_ms": 280, "min_speech_ms": 120,
        "max_silence_ms": 800, "trail_pad_ms": 160,
        "realtime_chunk_sec": 4.0, "flush_timeout": 1.4
    },
...
async def ws_handler(request: web.Request):
    ...
    pre_speech_frames = deque(maxlen=int(vad_cfg.get("pre_speech_ms", 280) / FRAME_MS))
    min_speech_bytes = int(vad_cfg.get("min_speech_ms", 120) / 1000 * SAMPLE_RATE * 2)
    max_silence_frames = int(vad_cfg.get("max_silence_ms", 800) / FRAME_MS)
    silence_counter = 0
    raw_audio_buffer = bytearray()
    
    realtime_chunk_bytes = int(vad_cfg.get("realtime_chunk_sec", 4.0) * SAMPLE_RATE * 2)
        ...
        while not ws.closed:
            try:
                ...
                if msg.type == web.WSMsgType.BINARY:
                    raw_audio_buffer.extend(msg.data[4:])
                    
                    while len(raw_audio_buffer) >= BYTES_PER_FRAME:
                        frame = raw_audio_buffer[:BYTES_PER_FRAME]
                        ...
                        if is_speaking:
                            ...
                            if prob >= VAD_ON:
                                is_speaking = True
                                silence_counter = 0
                                audio_buffer.extend(b"".join(pre_speech_frames))
                        
                        if is_speaking:
                            audio_buffer.extend(frame)
                        
                        if is_speaking and len(audio_buffer) >= realtime_chunk_bytes:
                            chunk_to_process = audio_buffer[:realtime_chunk_bytes]
                            audio_buffer = audio_buffer[realtime_chunk_bytes:]

                            arr16 = np.frombuffer(bytes(chunk_to_process), dtype=np.int16)
                        ...
                        if prev_is_speaking and not is_speaking and len(audio_buffer) >= min_speech_bytes:
                            arr16 = np.frombuffer(bytes(audio_buffer), dtype=np.int16)
                            audio_buffer.clear()
                ...
                # [오류 수정] 누락되었던 TEXT 메시지 처리 로직을 완벽하게 복원합니다.
                    ...
                    if data.get("type") == "get_config":
                        ...
                        if "whisper" in opts:
                            client_whisper_opts.update(client_config["whisper"])
                        if "vad" in opts:
                            vad_cfg = client_config["vad"]
                            ...
                            realtime_chunk_bytes = int(vad_cfg.get("realtime_chunk_sec", 4.0) * SAMPLE_RATE * 2)
                            flush_timeout = vad_cfg.get("flush_timeout", 1.4)
                            logging.info(f"VAD config updated.")
