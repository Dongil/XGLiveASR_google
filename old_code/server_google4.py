...
# [수정] API 키를 전역 변수로만 로드하고, 인스턴스 생성은 나중에 수행
...
# [제거] 전역 TRANSLATORS 딕셔너리 및 초기화 로직 제거
...
# (이하 load_config, deep_update, cors_mw 등은 기존과 동일)
def deep_update(d, u):
    for k, v in u.items():
...
async def ws_handler(request: web.Request):
    ...
    async def send_stt_result(text: str, is_final: bool, utterance_id: str):
        text = text.strip()
        if not text or ws.closed: return
        response = {"type": "stt", "utterance_id": utterance_id, "text": text, "is_final": is_final}
        try:
            await ws.send_str(json.dumps(response, ensure_ascii=False))
        except Exception as e:
            logging.warning(f"[{log_id}] Could not send STT message: {e}")

    # [수정] 번역 결과를 별도로 전송하는 함수 (내부에 동적 초기화 로직 추가)
    async def send_translation_result(original_text: str, utterance_id: str):
        original_text = original_text.strip()
        if not original_text or ws.closed: return
        ...
        # 번역 엔진 이름과 번역할 언어가 있을 때만 로직 실행
        if engine_name and target_langs:
            translator = None
            try:
                # 요청 시점에 필요한 번역기 인스턴스를 동적으로 생성
                ...
                logging.error(f"[{log_id}] Translator initialization failed for '{engine_name}': {e}")
            ...
            if translator:
                ...
                if tasks:
                 ...
                 logging.warning(f"[{log_id}] Translator '{engine_name}' is not available or configured.")
        ...
        try:
            ...
            logging.warning(f"[{log_id}] Could not send translation message: {e}")

    # (이하 google_stream_manager, google_stream_processor, 메인 루프 등은 이전 버전과 동일)
        ...
        try:
            ...
            async for response in stream:
                ...
                if result.is_final:
                    ...
                    await send_stt_result(transcript, is_final=True, utterance_id=current_utterance_id)
                    print(f"[{log_id}] RECV (Final): {transcript}")
                    
                    translation_buffer += transcript + " "
                    ...
                    if len(sentences) > 1:
                        ...
                        for sentence in sentences_to_translate:
                           await send_translation_result(sentence.strip(), str(uuid.uuid4()))
                    
                    current_utterance_id = None
                else:
                    if not current_utterance_id:
                        current_utterance_id = str(uuid.uuid4())
                    await send_stt_result(transcript, is_final=False, utterance_id=current_utterance_id)
        finally:
    ...
    try:
        while not ws.closed:
            ...
            if msg.type == web.WSMsgType.BINARY:
                ...
                if data.get("type") == "get_config": await send_config()
                    ...
                    opts = data.get("options") or {} # [수정] config 변경 로직 추가
        ...
        if translation_buffer.strip():
            await send_translation_result(translation_buffer.strip(), str(uuid.uuid4()))
        google_task.cancel()
        try:
            await google_task
        except asyncio.CancelledError:
