# 버전 28-rec2-hotwords-fix - 'hotwords'를 독립적인 옵션으로 처리하도록 수정
import asyncio
import json
import os
import ssl
...
DEFAULT_CONFIG = {
    "whisper": {
        ...
        "log_prob_threshold": -0.6, "compression_ratio_threshold": 1.6, "beam_size": 5, "best_of": 1,
        "no_speech_threshold": 0.6, "initial_prompt": None,
        # [신규] hotwords 기본값(빈 리스트) 추가. JSON에서는 리스트, 전달 시에는 문자열로 변환.
        "hotwords": []
    },
    "vad": {
...
async def ws_handler(request: web.Request):
    ...
    # [수정 시작] 'hotwords'를 올바르게 처리하는 로직으로 변경
    def prepare_whisper_opts(config_whisper: dict) -> dict:
        """설정 객체를 받아 faster-whisper가 요구하는 형태로 hotwords를 변환하는 함수"""
        opts = copy.deepcopy(config_whisper)
        
        # hotwords가 리스트 형태일 경우, 쉼표로 구분된 문자열로 변환
        if "hotwords" in opts and isinstance(opts["hotwords"], list):
            hotwords_list = opts["hotwords"]
            opts["hotwords"] = ", ".join(hotwords_list)
            logging.info(f"[{log_id}] Hotwords list converted to string: '{opts['hotwords']}'")
        
        # hotwords가 있지만 비어있는 경우(None, "", []) None으로 설정하여 불필요한 파라미터 전달 방지
        if not opts.get("hotwords"):
            opts["hotwords"] = None
            
        return opts

    client_whisper_opts = prepare_whisper_opts(client_config["whisper"])
    # [수정 끝]
    ...
    async def send_audio_file(raw_audio_data: bytes):
        if not raw_audio_data or ws.closed:
            response = {"type": "audio_file", "filename": None, "data": None}
            await ws.send_str(json.dumps(response, ensure_ascii=False))
            print(f"[{log_id}] No VAD audio recorded, sending empty signal to client.")
            return
        
        print(f"[{log_id}] Creating WAV file from {len(raw_audio_data)} bytes of VAD audio.")
        try:
            ...
            response = {
                "type": "audio_file",
                "filename": f"vad_processed_{log_id}.wav",
                "data": encoded_audio
            }
            
            await ws.send_str(json.dumps(response, ensure_ascii=False))
            print(f"[{log_id}] Sent VAD audio file to client for download.")
            
        except Exception as e:
            logging.error(f"[{log_id}] Failed to create or send WAV file: {e}")
    ...
    async def silence_watcher():
        ...
        while not ws.closed:
            ...
            should_record = client_config.get("processing", {}).get("record_server_audio", False)
            now = loop.time()
                    ...
                    if (now - last_audio_time > flush_timeout) and (collector.collected or transcribe_buffer or sentence_buffer):
                        ...
                        if collector.collected:
                            ...
                            if chunk:
                                ...
                                if should_record: audio_record_buffer.extend(chunk)
                        ...
                        await send_stt(text_fragment, is_final_flush=True)
                        last_audio_time = now
    ...
    try:
        while not ws.closed:
            ...
            if msg.type == web.WSMsgType.BINARY:
                ...
                while len(buf) >= BYTES_PER_FRAME:
                    ...
                    if len(transcribe_buffer) >= MIN_CHUNK_BYTES:
                        async with processing_lock:
                            ...
                            text_fragment = "".join(s.text for s in segments).strip()
                            text_fragment = re.sub(r'[^\x20-\x7E가-힣\s.,?!]', '', text_fragment)
                            if text_fragment: await send_stt(text_fragment)
                ...
                if data.get("type") == "get_config": await send_config()
                elif data.get("type") == "get_audio_file":
                    if collector.collected and len(collector.collected) > collector.min_frames:
                        chunk = collector._finalize()
                        if chunk:
                            if should_record: audio_record_buffer.extend(chunk)
                    await send_audio_file(bytes(audio_record_buffer))
                    ...
                    # [수정 시작] 설정 변경 시 whisper 옵션을 올바르게 다시 준비
                    if "whisper" in opts:
                        client_whisper_opts = prepare_whisper_opts(client_config["whisper"])
                    # [수정 끝]
                    ...
                    if "processing" in opts:
                        use_kss = client_config.get("processing", {}).get("use_kss")
                        record = client_config.get("processing", {}).get("record_server_audio")
                        logging.info(f"[{log_id}] Processing config updated: use_kss={use_kss}, record_server_audio={record}")
