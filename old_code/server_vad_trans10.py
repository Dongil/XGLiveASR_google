...
DEFAULT_CONFIG = {
    "whisper": {
        ...
        "log_prob_threshold": -0.35, "compression_ratio_threshold": 1.6, "beam_size": 4, "best_of": 1,
    ...
    "vad": {
        ...
        "max_silence_ms": 500, "trail_pad_ms": 160, "max_segment_sec": 7,
            ...
            try:
                ...
                if msg.type == web.WSMsgType.BINARY:
                    ...
                    while len(buf) >= BYTES_PER_FRAME:
                        ...
                        # [수정] '적응형 버퍼 플러시' 로직 적용
                        if (chunk or not state_speech) and len(transcribe_buffer) > 0:
                            # 버퍼가 최소 길이를 넘었거나, 또는 말이 끝난 상태이고 버퍼에 데이터가 남아있다면 즉시 처리
                            if len(transcribe_buffer) >= MIN_CHUNK_BYTES or not state_speech:
