# 버전 32.3 - 서버 측 번역 버퍼링 및 클라이언트 UI 문제 최종 해결
import asyncio
import json
import os
import ssl
...
async def ws_handler(request: web.Request):
    ...
    async def send_translation_result(original_text: str, sentence_id: str):
        ...
        if engine_name and target_langs:
            ...
            if translator: translations = await asyncio.gather(*[translator.translate(original_text, lang) for lang in target_langs])
            else: logging.warning(f"[{log_id}] Translator '{engine_name}' not available.")
        
        await send_json({"type": "translation_update", "sentence_id": sentence_id, "translations": translations})
    ...
    async def google_stream_processor():
        ...
        try:
            ...
            streaming_config = speech.StreamingRecognitionConfig(
                config=speech.RecognitionConfig(encoding=speech.RecognitionConfig.AudioEncoding.LINEAR16, sample_rate_hertz=SAMPLE_RATE, **stt_config),
            ...
            async for response in stream:
                ...
                if result.is_final:
                    final_piece = transcript[len(full_transcript):].strip()
                    if final_piece:
                        sentence_id = str(uuid.uuid4())
                        await send_json({"type": "stt_final_piece", "sentence_id": sentence_id, "text": final_piece})
                        ...
                        if len(sentences) > 1:
                            for sentence in sentences[:-1]:
                                await send_translation_result(sentence.strip(), sentence_id)
