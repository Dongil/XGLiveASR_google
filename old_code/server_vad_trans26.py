# 버전 26 - 사용자별(ID별) 설정 파일 로드 및 저장 기능 추가
import asyncio
import json
import os
import ssl
...
USER_CONFIG_DIR = "user_configs" # [신규] 사용자별 설정파일 저장 디렉토리
os.makedirs(USER_CONFIG_DIR, exist_ok=True) # [신규] 디렉토리 자동 생성
...
# [수정] load_config -> load_global_config 로 이름 변경하여 역할 명확화
...
# [신규] 사용자 ID를 기반으로 최종 설정을 로드하는 함수
def load_user_config(user_id: Optional[str]):
    # 1. 전역 설정을 기본으로 로드
    cfg = load_global_config()
    
    # 2. 사용자 ID가 있으면, 해당 ID의 설정 파일을 덮어씌움
    if user_id:
...
# ---------- Whisper 모델 로딩 및 '예열' (전역으로 한 번만 실행) ----------
...
async def ws_handler(request: web.Request):
    ...
    try:
        ...
        # ... (finally 블록은 25.2와 동일, 로그 ID만 변경) ...
        watcher_task.cancel()
        try:
            await watcher_task
