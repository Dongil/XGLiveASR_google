# 버전 32 - 서버 측 번역 버퍼링 및 클라이언트 UI 문제 해결 지원
import asyncio
import json
import os
import ssl
...
async def ws_handler(request: web.Request):
    ...
    # [수정] process_stt_result -> send_stt_result 로 이름 변경 및 로직 단순화
    ...
    async def google_stream_processor():
        ...
        current_utterance_id = None # 각 스트림마다 고유 ID 관리

        try:
            if adaptation_config and adaptation_config.get("phrases"):
                # (이전과 동일한 Adaptation 로직)
            ...
            async for response in stream:
                ...
                if result.is_final:
                    if not current_utterance_id: # 이전 문장이 interim 없이 바로 final로 끝난 경우
                    ...
                    # [수정] 번역 버퍼에 최종 텍스트 추가 및 KSS로 문장 분리
                    ...
                    if len(sentences) > 1:
                        ...
                        # 완성된 문장들을 순차적으로 번역 요청
                        for sentence in sentences_to_translate:
                           await send_translation_result(sentence.strip(), str(uuid.uuid4())) # 번역 결과는 고유 ID를 가짐
                    
                    current_utterance_id = None # 다음 문장을 위해 ID 초기화
